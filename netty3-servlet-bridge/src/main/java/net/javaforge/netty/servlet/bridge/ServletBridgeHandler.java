...
import net.javaforge.netty.handler.HttpStaticFileHandler;
...
public class ServletBridgeHandler extends IdleStateAwareChannelHandler {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
        ...
        if (uri.contains("/img")) {
            new HttpStaticFileHandler().messageReceived(ctx, e);
            return;
        }

        FilterChainImpl chain = ServletBridgeWebapp.get().initializeChain(request.getUri());
    ...
    protected void handleHttpServletRequest(ChannelHandlerContext ctx,
        ...
        if (keepAlive) {
            ...
            headers.add(CONTENT_LENGTH, response.getContent().readableBytes());
            // Add keep alive header as per:
            // -
            // http://www.w3.org/Protocols/HTTP/1.1/draft-ietf-http-v11-spec-01.html#Connection
            headers.add(CONNECTION, HttpHeaders.Values.KEEP_ALIVE);
        }
